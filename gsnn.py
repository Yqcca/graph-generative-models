...
class GSNLayer(MessagePassingBase):
    ...
    def __init__(self, input_dim, max_cycle=8, edge_input_dim=None,mlp_hidden_dims=None,
        ...
        self.msg_mlp = layers.MLP(2 * input_dim + 2 * self.node_counts_dim,
    ...
    def message(self, graph, input):
        ...
        if graph.num_edge:
            message = torch.cat([input[node_in], input[node_out],
                                 ...
                                 graph.node_structural_feature[node_out].float()], dim=-1)
...
class GSN(nn.Module, core.Configurable):
    ...
    """
    def __init__(self, input_dim, hidden_dim, num_relation, num_layer, num_mlp_layer=2, max_cycle=8, edge_input_dim=None, 
        ...
        for i in range(num_layer):
            self.layers.append(GSNLayer(hidden_dim, max_cycle, edge_input_dim, [hidden_dim] * (num_mlp_layer - 1), batch_norm, activation))
